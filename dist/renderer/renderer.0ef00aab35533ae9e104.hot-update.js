    ...
    switch (action.type) {
        ...
        default: {
            ...
            if (isUndoable(action)) {
                ...
                const finalState = (0,immer__WEBPACK_IMPORTED_MODULE_5__.produce)(stateCopy, draft => {
                    // Create a fresh copy of the next state
                    const nextStateCopy = JSON.parse(JSON.stringify(nextState));
                    // Store selection state before any updates
                    const prevSelectedClipIds = Array.from(nextStateCopy.selectedClipIds || []);
                    // Update state properties
                    draft.tracks = nextStateCopy.tracks;
                    draft.selectedClipIds = nextStateCopy.selectedClipIds;
                    draft.currentTime = nextStateCopy.currentTime;
                    draft.duration = nextStateCopy.duration;
                    draft.zoom = nextStateCopy.zoom;
                    draft.fps = nextStateCopy.fps;
                    draft.isPlaying = nextStateCopy.isPlaying;
                    draft.isDragging = nextStateCopy.isDragging;
                    draft.dragStartX = nextStateCopy.dragStartX;
                    draft.dragStartY = nextStateCopy.dragStartY;
                    draft.scrollX = nextStateCopy.scrollX;
                    draft.scrollY = nextStateCopy.scrollY;
                    draft.error = nextStateCopy.error;
                    draft.selectedTrackId = nextStateCopy.selectedTrackId;
                    // Update selection state based on clip existence
                    const selectedClipsExist = prevSelectedClipIds.some(id => nextStateCopy.tracks.some(track => track.clips.some(clip => clip.id === id)));
                    if (!selectedClipsExist && nextStateCopy.tracks.length > 0) {
                        const firstTrack = nextStateCopy.tracks[0];
                        if (firstTrack.clips.length > 0) {
                            draft.selectedClipIds = [firstTrack.clips[0].id];
                        }
                        else {
                        ...
                        draft.selectedClipIds = prevSelectedClipIds;
                    }
                    // Update history
                    // Create deep copies of patches to avoid any proxy issues
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("e3d2e3b24da5c562adad")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=renderer.0ef00aab35533ae9e104.hot-update.js.map