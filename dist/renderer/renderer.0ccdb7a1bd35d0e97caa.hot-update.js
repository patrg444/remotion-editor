    ...
    switch (action.type) {
        ...
        case _types_timeline__WEBPACK_IMPORTED_MODULE_1__.ActionTypes.UNDO: {
            if (state.history.currentIndex > 0) {
                ...
                // Log patches to be applied
                _utils_logger__WEBPACK_IMPORTED_MODULE_3__.logger.debug('[UNDO] Patches to apply:', {
                    patchCount: inversePatches.length,
                    patches: inversePatches.map(p => ({
                        op: p.op,
                        path: p.path,
                        value: p.value
                    }))
                ...
                // Find clips that were affected by the patches
                const clipPatches = inversePatches.filter(p => p.path.includes('/clips/'));
                if (clipPatches.length > 0) {
                    _utils_logger__WEBPACK_IMPORTED_MODULE_3__.logger.debug('[UNDO] Clip patches applied:', {
                        clipPatches: clipPatches.map(p => ({
                            op: p.op,
                            path: p.path,
                            value: p.value
                        }))
                ...
                // Log final clip state
                finalState.tracks.forEach(track => {
                    track.clips.forEach(clip => {
                        _utils_logger__WEBPACK_IMPORTED_MODULE_3__.logger.debug('[UNDO] Final clip state:', {
                            clipId: clip.id,
        ...
        case _types_timeline__WEBPACK_IMPORTED_MODULE_1__.ActionTypes.REDO: {
            if (state.history.currentIndex < state.history.entries.length - 1) {
                ...
                // Log patches to be applied
                _utils_logger__WEBPACK_IMPORTED_MODULE_3__.logger.debug('[REDO] Patches to apply:', {
                ...
                if (clipPatches.length > 0) {
                    _utils_logger__WEBPACK_IMPORTED_MODULE_3__.logger.debug('[REDO] Clip patches applied:', {
                        clipPatches: clipPatches.map(p => ({
                            op: p.op,
                            path: p.path,
                            value: p.value
                ...
                finalState.tracks.forEach(track => {
                    track.clips.forEach(clip => {
                        _utils_logger__WEBPACK_IMPORTED_MODULE_3__.logger.debug('[REDO] Final clip state:', {
                            clipId: clip.id,
                            startTime: clip.startTime,
                            endTime: clip.endTime,
                            layer: clip.layer,
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("23e43d1652160fa75a81")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=renderer.0ccdb7a1bd35d0e97caa.hot-update.js.map