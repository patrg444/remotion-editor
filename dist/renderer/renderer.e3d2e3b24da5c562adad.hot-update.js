        ...
        default: {
            ...
            if (isUndoable(action)) {
                ...
                const finalState = (0,immer__WEBPACK_IMPORTED_MODULE_5__.produce)(stateCopy, draft => {
                    ...
                    // Create a new state object with all properties except history
                    const newState = {
                        tracks: nextStateCopy.tracks,
                        selectedClipIds: nextStateCopy.selectedClipIds,
                        currentTime: nextStateCopy.currentTime,
                        duration: nextStateCopy.duration,
                        zoom: nextStateCopy.zoom,
                        fps: nextStateCopy.fps,
                        isPlaying: nextStateCopy.isPlaying,
                        isDragging: nextStateCopy.isDragging,
                        dragStartX: nextStateCopy.dragStartX,
                        dragStartY: nextStateCopy.dragStartY,
                        scrollX: nextStateCopy.scrollX,
                        scrollY: nextStateCopy.scrollY,
                        error: nextStateCopy.error,
                        selectedTrackId: nextStateCopy.selectedTrackId
                    };
                    // Update draft with new state
                    Object.assign(draft, newState);
                    // Update selection state based on clip existence
                    const selectedClipsExist = prevSelectedClipIds.some(id => newState.tracks.some(track => track.clips.some(clip => clip.id === id)));
                    if (!selectedClipsExist && newState.tracks.length > 0) {
                        const firstTrack = newState.tracks[0];
                        if (firstTrack.clips.length > 0) {
                            draft.selectedClipIds = [firstTrack.clips[0].id];
                        }
                        else {
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("30e3f3e1c8904a4dd560")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=renderer.e3d2e3b24da5c562adad.hot-update.js.map