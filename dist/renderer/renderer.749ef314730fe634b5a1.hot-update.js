    ...
    switch (action.type) {
        ...
        case _types_timeline__WEBPACK_IMPORTED_MODULE_1__.ActionTypes.UNDO: {
            if (state.history.currentIndex > 0) {
                ...
                const undone = (0,immer__WEBPACK_IMPORTED_MODULE_4__.applyPatches)(state, inversePatches);
                ...
                if (clipPatches.length > 0) {
                    _utils_logger__WEBPACK_IMPORTED_MODULE_3__.logger.debug('[UNDO] Applying clip patches:', {
        ...
        case _types_timeline__WEBPACK_IMPORTED_MODULE_1__.ActionTypes.REDO: {
            if (state.history.currentIndex < state.history.entries.length - 1) {
                ...
                // Apply patches directly to state
                const redone = (0,immer__WEBPACK_IMPORTED_MODULE_4__.applyPatches)(state, patches);
                // Find and log only clip-related patches
                const clipPatches = patches.filter(p => p.path.includes('/clips/'));
                if (clipPatches.length > 0) {
                    _utils_logger__WEBPACK_IMPORTED_MODULE_3__.logger.debug('[REDO] Applying clip patches:', {
                        clipCount: clipPatches.length,
                        patches: clipPatches.map(p => ({
                            op: p.op,
                            path: p.path,
        ...
        default: {
            ...
            const stateAfterPatches = (0,immer__WEBPACK_IMPORTED_MODULE_4__.applyPatches)(state, patches);
            // Only create history entry for undoable actions
            if (isUndoable(action)) {
                // Create a new state with history updates
                const finalState = (0,immer__WEBPACK_IMPORTED_MODULE_4__.produce)(stateAfterPatches, draft => {
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("45b2478d3f6909d93fcb")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=renderer.749ef314730fe634b5a1.hot-update.js.map