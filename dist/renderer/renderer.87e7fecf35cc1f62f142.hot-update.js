...
const TimelineClip = ({ clip, track, layer, zoom, fps, onSelect, onDragStart, onDragEnd, tabIndex = 0, 'aria-posinset': posinset, 'aria-setsize': setsize, style }) => {
    ...
    // Handle clip rendering and positioning
    (0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
        if (!clipRef.current)
            return;
        _utils_logger__WEBPACK_IMPORTED_MODULE_12__.logger.debug('[TimelineClip] Updating clip position:', {
            clipId: clip.id,
        ...
        const top = clip.layer * _utils_timelineConstants__WEBPACK_IMPORTED_MODULE_7__.TimelineConstants.UI.TRACK_HEIGHT;
        ...
        requestAnimationFrame(() => {
            ...
            _utils_logger__WEBPACK_IMPORTED_MODULE_12__.logger.debug('[TimelineClip] Clip positioned:', {
                clipId: clip.id,
                rect,
    ...
    (0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
        ...
        if (propsChanged) {
            ...
            _utils_logger__WEBPACK_IMPORTED_MODULE_12__.logger.debug('[TimelineClip] Props changed:', {
                clipId: clip.id,
                changes: {
                    id: clip.id !== lastPropsRef.current.id,
                    startTime: clip.startTime !== lastPropsRef.current.startTime,
                    endTime: clip.endTime !== lastPropsRef.current.endTime,
                    layer: layer !== lastPropsRef.current.layer
                }
            });
        }
        if (!hasEmittedRef.current && clipRef.current) {
            _utils_logger__WEBPACK_IMPORTED_MODULE_12__.logger.debug('[TimelineClip] Starting render sequence:', {
                clipId: clip.id,
                startTime: clip.startTime,
                endTime: clip.endTime,
                layer,
            ...
            requestAnimationFrame(() => {
                if (clipRef.current) {
                    const rect = clipRef.current.getBoundingClientRect();
                    _utils_logger__WEBPACK_IMPORTED_MODULE_12__.logger.debug('[TimelineClip] Ready to dispatch rendered event:', {
                        clipId: clip.id,
                        rect,
                        style: clipRef.current.style
                    });
                    ...
                    // Force another reflow before dispatching positioned event
                    void clipRef.current.offsetHeight;
                    // Dispatch positioned event in next frame
                    requestAnimationFrame(() => {
                        if (clipRef.current) {
                            const updatedRect = clipRef.current.getBoundingClientRect();
                            _utils_logger__WEBPACK_IMPORTED_MODULE_12__.logger.debug('[TimelineClip] Ready to dispatch positioned event:', {
                                clipId: clip.id,
                                rect: updatedRect,
                                style: clipRef.current.style
                            });
                            ...
                            _utils_logger__WEBPACK_IMPORTED_MODULE_12__.logger.debug('[TimelineClip] Render sequence complete:', {
                                clipId: clip.id,
                                hasEmitted: hasEmittedRef.current
                            });
                        }
                    });
                }
        ...
        return () => {
            ...
            if (propsChanged) {
                hasEmittedRef.current = false;
                _utils_logger__WEBPACK_IMPORTED_MODULE_12__.logger.debug('[TimelineClip] Cleanup:', {
                    clipId: clip.id,
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("706e0ba66f6e04d04209")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=renderer.87e7fecf35cc1f62f142.hot-update.js.map