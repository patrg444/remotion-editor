    ...
    switch (action.type) {
        ...
        case _types_timeline__WEBPACK_IMPORTED_MODULE_1__.ActionTypes.UNDO: {
            if (state.history.currentIndex > 0) {
                ...
                // Create a copy of state to avoid modifying history
        ...
        default: {
            ...
            if (isUndoable(action)) {
                ...
                if (nextState.history.currentIndex < nextState.history.entries.length - 1) {
                    nextState.history.entries = nextState.history.entries.slice(0, nextState.history.currentIndex + 1);
                }
                // Push the patches and inversePatches with type and timestamp
                const entry = {
                    type: 'partial',
                    timestamp: Date.now(),
                    patches,
                ...
                nextState.history.entries.push(entry);
                nextState.history.currentIndex = nextState.history.entries.length - 1;
                // Trim history if it exceeds max size
                if (nextState.history.entries.length > _utils_timelineConstants__WEBPACK_IMPORTED_MODULE_2__.TimelineConstants.History.MAX_HISTORY_SIZE) {
                    nextState.history.entries = nextState.history.entries.slice(-_utils_timelineConstants__WEBPACK_IMPORTED_MODULE_2__.TimelineConstants.History.MAX_HISTORY_SIZE);
                    nextState.history.currentIndex = nextState.history.entries.length - 1;
                }
                _utils_logger__WEBPACK_IMPORTED_MODULE_3__.logger.debug('Added history entry:', {
                    description: entry.description,
                    currentIndex: nextState.history.currentIndex,
                    totalEntries: nextState.history.entries.length
                });
            }
            return nextState;
        }
...
const TimelineProvider = ({ children }) => {
    ...
    (0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
        try {
            ...
            // Dispatch a ready event so tests know when it's safe to use
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("a643a3a3472a2cf259e6")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=renderer.c6365ea61b1cdc45bbe8.hot-update.js.map