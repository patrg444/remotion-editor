...
const CHECKPOINT_ACTIONS = new Set([
    ...
    'MOVE_TRACK',
    'MOVE_CLIP',
    'SPLIT_CLIP' // Make split a checkpoint since it's a complex operation
]);
// Actions that should create a history entry but aren't checkpoints
...
const timelineReducer = (state, action) => {
    return (0,immer__WEBPACK_IMPORTED_MODULE_6__.produce)(state, draft => {
        ...
        if (action.type !== _types_timeline__WEBPACK_IMPORTED_MODULE_1__.ActionTypes.UNDO && action.type !== _types_timeline__WEBPACK_IMPORTED_MODULE_1__.ActionTypes.REDO) {
            // Create history entries for checkpoint actions and history actions
            if (CHECKPOINT_ACTIONS.has(action.type) || HISTORY_ACTIONS.has(action.type)) {
            ...
            case _types_timeline__WEBPACK_IMPORTED_MODULE_1__.ActionTypes.SPLIT_CLIP:
                {
                    ...
                    if (trackToSplit) {
                        ...
                        if (clipToSplit && action.payload.time > clipToSplit.startTime && action.payload.time < clipToSplit.endTime) {
                            ...
                            // Store the original clip for history
                            const originalClip = { ...clipToSplit };
                            // Log state before split
                            _utils_logger__WEBPACK_IMPORTED_MODULE_4__.logger.debug('Before split:', {
                                originalClip,
                                allClips: trackToSplit.clips
                            });
                            // Remove the original clip
                            trackToSplit.clips = trackToSplit.clips.filter(c => c.id !== clipToSplit.id);
                            // Log state after removal
                            _utils_logger__WEBPACK_IMPORTED_MODULE_4__.logger.debug('After removal:', {
                                remainingClips: trackToSplit.clips
                            });
                            // Create first clip
                            const firstClip = {
                                ...clipToSplit,
                            ...
                            // Update selection and ripple state in a single operation
                            draft.selectedClipIds = [firstClip.id, secondClip.id];
                            // Clear rippleState for the original clip and initialize for new clips
                            ...
                            // Log final state
                            _utils_logger__WEBPACK_IMPORTED_MODULE_4__.logger.debug('After split:', {
                                firstClip,
                                secondClip,
                                allClips: trackToSplit.clips,
                                selectedClipIds: draft.selectedClipIds,
                                rippleState: draft.rippleState
                            });
                        }
                    }
                }
        ...
        if (shouldCreateHistoryEntry) {
            ...
            const hasChanges = diff.type === 'full' || (diff.changes && Object.keys(diff.changes).length > 0);
            if (hasChanges) {
                ...
                _utils_logger__WEBPACK_IMPORTED_MODULE_4__.logger.debug('Added history entry:', {
                    description: historyDescription,
                    currentIndex: draft.history.currentIndex,
                    totalEntries: draft.history.entries.length
                });
            }
        }
    });
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("f48a06da1411c408512f")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=renderer.5f1060e3781bbf22d0cf.hot-update.js.map